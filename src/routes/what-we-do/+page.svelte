	...
	onMount(() => {
		if (browser) {
			...
			const ctx = gsap.context((self) => {
				const philosophy = SplitText.create('.philosophy', {
					type: 'chars,words',
					smartWrap: true
				});
...
<section class="w-full border-y border-gray-300 bg-gray-100 md:px-8">
	<div class="relative mx-auto flex flex-col items-center justify-center p-16 text-gray-800">
		<p
			...
			What We Do
			<br />
			<span class="mt-2 inline-block text-sm font-semibold text-gray-700">
				Without the jargon.
			</span>
		</p>
		...
		<button
			...
			class="md:text-md hover:bg-primary group relative mx-auto mb-8 w-fit cursor-pointer rounded-full border-2 border-black bg-gray-50 p-2 px-4 text-xs font-medium text-gray-800 transition-all duration-200 hover:text-white active:scale-95 sm:p-3 sm:px-6 sm:text-sm">
			Scroll To Learn More
			<span
				class="bg-secondary absolute right-0 -bottom-4 left-0 mx-auto inline-flex size-6 items-center justify-center rounded-full border-2 border-black text-black transition-all duration-200 group-hover:-bottom-5 group-hover:text-black md:-bottom-5 md:size-8 md:group-hover:-bottom-6">
				<Icon icon="ph:arrow-down-bold" class="inline h-4 w-4 align-middle md:h-5 md:w-5" />
			</span>
		</button>

		<h3
			class="philosophy text-2xl leading-12 font-medium tracking-tight md:w-2/3 md:text-3xl md:leading-15 lg:text-4xl">
			At <span class="text-primary">Wurks Studio</span>, we build websites that make your business
			look trustworthy, modern, and ready for customers. No tech talk, no confusing features.
